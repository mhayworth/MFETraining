...
const UserSearch = ({ country, setCountry, search, setSearch }: UserSearchProps): React.ReactElement => {
    ...
    return (
        <>
            <div style={{ padding: "1em", display: "inline-flex" }}>
                <form onSubmit = {(event) => {event.preventDefault(); setSearch(searchInput); navigate(`/find/${searchInput}?${searchParams.toString()}`)}}>
                    <p style={{ marginTop: "auto", marginBottom: "auto", marginLeft: "0.75em", marginRight: "0.75em" }}>Order ID:</p>
                    <input value={searchInput} required={true} type="text" id="order-id-input" className={(searchTouched && (searchInput.includes(" ")))?"input-error":""} style={{ padding: "0.25em" }} onChange={(event) => handleChange(event)} onKeyDown={(event) => { if (event.key === 'Enter') { setSearch(searchInput); navigate(`/find/${searchInput}${searchParams.toString()}`); } }} />
