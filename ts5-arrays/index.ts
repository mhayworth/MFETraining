{
    ...
    const transactionArea: HTMLDivElement = document.getElementById("transactions-list") as HTMLDivElement;
    ...
    const addTransaction = (): void => {
        ...
        const amount: number = parseInt((<HTMLInputElement>document.getElementById("amount")).value)
        userBalance = processTransaction({description: description, amount: amount, date: new Date()})
        document.getElementById("balance").innerHTML = String(userBalance);
    }
    ...
    const processTransaction = (transaction: TransactionType): number => {
        ...
        transactionArray.map(transaction => {
            innerHtml += `<li>${transaction.date}, ${transaction.description}, ${transaction.amount}</li>`;
        }) 
        innerHtml += `</ul>`
    ...
    const [name1, name2] = exampleArray //will be first and second value
    const exampleArrayCopy = [...exampleArray, "Meow"] //will be value copy of array
}